...
CONFIG_DATA = load_yaml(MAIN_DIRECTORY / "tools" / "config.yaml")
...
def load_theme():
    print("[FLOW] load_theme() called")
    global THEME_DATA
    try:
        print(f"[FLOW] Getting theme name from CONFIG_DATA: {CONFIG_DATA['config']['THEME']}")
        theme_path = Path("res/themes/" + CONFIG_DATA['config']['THEME'])
        print(f"[FLOW] Theme path: {theme_path}")
        logger.info("Loading theme %s from %s" % (CONFIG_DATA['config']['THEME'], theme_path / "theme.yaml"))
        print(f"[FLOW] About to load YAML from: {MAIN_DIRECTORY / theme_path / 'theme.yaml'}")
        THEME_DATA = load_yaml(MAIN_DIRECTORY / theme_path / "theme.yaml")
        print("[FLOW] YAML loaded successfully")
        THEME_DATA['PATH'] = str(MAIN_DIRECTORY / theme_path) + "/"
        print("[FLOW] THEME_DATA path set")
    except Exception as e:
        print(f"[FLOW] Exception in load_theme(): {e}")
    ...
    print("[FLOW] Theme loading completed successfully")
...
# Photo frame application - no theme loading needed
print("[FLOW] Photo frame mode - skipping theme loading")
print("[FLOW] THEME_DATA remains None")
