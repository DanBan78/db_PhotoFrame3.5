...
from lib.lcd.serialize import image_to_RGB565, chunked
...
class LcdCommRevA(LcdComm):
    ...
    def DisplayPILImage(
        ...
        # Small delay to let LCD prepare for incoming bitmap data
        import time
        ...
        with self.update_queue_mutex:
            ...
            import time
            for i, chunk in enumerate(chunked(rgb565le, width * 8)):
                self.SendLine(chunk)
                # Add small delay every few chunks to prevent LCD buffer overflow
